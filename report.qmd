...
This report summarises what the current dataset can tell us about when and where African hominins appear in the record. It is written for paleoarchaeologists who may not use statistical tools day‑to‑day.
...
The emphasis here is on clear visuals and practical interpretation.
...
top <- head(sf, 5)
bul <- apply(top, 1, function(row) sprintf("- %s: %s specimens", row[["species"]], row[["n_specimens"]]))
...
bul <- apply(top, 1, function(r) sprintf(
  "- %s spans ~%0.2f Ma (%0.2f–%0.2f Ma)",
  r[["species"]], as.numeric(r[["duration"]]), as.numeric(r[["last_occ"]]), as.numeric(r[["first_occ"]])
))
cat(paste("Longest spans (point estimates):", paste(bul, collapse = "\n"), sep = "\n"))
```
...
bul <- apply(top_wide, 1, function(r) sprintf(
  "- %s: median duration %0.2f Ma (90%% interval %0.2f–%0.2f Ma)",
  r[["species"]], as.numeric(r[["dur_med"]]), as.numeric(r[["dur_lo"]]), as.numeric(r[["dur_hi"]])
))
cat(paste("Widest duration uncertainty (MC):", paste(bul, collapse = "\n"), sep = "\n"))
```
...
- Near‑zero overlap may mean genuine turnover or simply gaps in sampling.
...
bul <- apply(top, 1, function(r) sprintf(
  "- %s with %s: ~%0.2f Ma",
  r[["species_i"]], r[["species_j"]], as.numeric(r[["overlap_Ma"]])
))
cat(paste("Largest point-estimated overlaps:", paste(bul, collapse = "\n"), sep = "\n"))
```
...
bul <- apply(top, 1, function(r) sprintf(
  ...
  r[["species_i"]], r[["species_j"]], as.numeric(r[["ov_med"]]), as.numeric(r[["ov_lo"]]), as.numeric(r[["ov_hi"]])
))
cat(paste("Largest overlaps with uncertainty:", paste(bul, collapse = "\n"), sep = "\n"))
```
...
cat(sprintf(
  ...
  nonzero,
  paste(apply(top_vel, 1, function(r) sprintf("%s (bin %0.2f): %0.0f km/Ma", r[["species"]], as.numeric(r[["bin"]]), as.numeric(r[["velocity_km_per_Ma"]]))), collapse = "; ")
))
```
...
ll <- leadlag_tbl
earliest <- dplyr::arrange(ll, delta_fad_median) |> head(3)
latest   <- dplyr::arrange(ll, dplyr::desc(delta_fad_median)) |> head(3)
bul1 <- apply(earliest, 1, function(r) sprintf("- Earliest FAD: %s in %s (%0.3f Ma earlier)", r[["species"]], r[["country"]], as.numeric(r[["delta_fad_median"]])) )
bul2 <- apply(latest,   1, function(r) sprintf("- Latest FAD: %s in %s (+%0.3f Ma)",       r[["species"]], r[["country"]], as.numeric(r[["delta_fad_median"]])) )
cat(paste(c("Highlights:", bul1, bul2), collapse = "\n"))
```
